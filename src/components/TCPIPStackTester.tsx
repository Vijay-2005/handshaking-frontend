...
export default function TCPIPStackTester() {
  // --- Backend API hooks ---------------------------------------------------
  const { mutateAsync: sendPacketApi } = useSendPacket();
  const { mutate: resetApi } = useReset();
  const { mutateAsync: runTestApi } = useRunTest();
  ...
  // Sync local state with backend status when it changes
  useEffect(() => {
    if (statusData) {
      // Attempt to map backend status into local state fields.
      // Fallback to existing state if structure is unexpected.
            const s: any = statusData as any;
      setCurrentState((s.current_state as FSMState) ?? currentState);

      const stats = s.statistics as any;
      if (stats) {
        setTestStats(prev => ({
          total: stats.total_packets ?? prev.total,
          pass: stats.passed_packets ?? prev.pass,
          fail: stats.failed_packets ?? prev.fail,
          inputCoverage: (s.coverage?.input_coverage_percentage as number) ?? prev.inputCoverage,
          fsmCoverage: (s.coverage?.fsm_coverage_percentage as number) ?? prev.fsmCoverage,
        }));
      }
    }
  }, [statusData]);
  ...
  const sendPacket = async () => {
    ...
    const payload = { data: dataIn, checksum: checksumIn, valid: validIn };
    try {
      await sendPacketApi(payload);
    } catch (err) {
      console.error(err);
